...
if (isEnabled) {
  try {
    ...
    Purchases.setLogHandler((logLevel, message) => {
      // Suppress "already set" warnings during development hot reloads
    ...
    // Check if SDK is already configured (prevent double-initialization during hot reloads)
    Purchases.isConfigured().then((configured) => {
      ...
      // If isConfigured check fails, try to configure anyway
      try {
        Purchases.configure({ apiKey: apiKey! });
        console.log(`${LOG_PREFIX} SDK initialized successfully`);
      } catch (configError) {
        // Silently ignore if already configured
        if (!String(configError).includes('already set')) {
          console.error(`${LOG_PREFIX} Failed to initialize:`, configError);
        }
      }
    });
  } catch (error) {
